...
function showTagInfo(tag){
    ...
    getTagItemsAsync(tag.id, showGoods.bind(tag));

}

function showGoods(err, goods){

    if(err){
        console.log(`[${err.message}]`);
    }
    else{
        console.log("Товары по тегу #" + this.title);
...
function getTagItemsCountAsync(title, callback){
    getTagAsync(title, tegInfo);
    function tegInfo(err, tag){
        if (err){
            console.log(err.message);
        }
        else{
            getTagItemsAsync(tag.id, tagItems);
        }
        function tagItems(err, goods){
            if (err){
                console.log(err.message);
            }

            callback(null, goods.length);
        }
    }
...
function getTagItemsCount(title){
    return getTag(title).then(tag => getTagItems(tag.id)).then(goods => goods.length);
}



getTagItemsCount(tagTitle)
    .then(count => console.log(`По промис-тегу #${tagTitle} найдено товаров ${count} шт.`))
